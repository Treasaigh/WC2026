...
ggplot(eloDataSm, aes(x=Year, y=Rating, color=Confederation)) +
...
#ZXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
...
matches %>%select(Date, Home_Team, Away_Team, Home_Team_Goals, Away_Team_Goals) %>%
  mutate(Total.Goals = Home_Team_Goals + Away_Team_Goals) %>%
  arrange(-Total.Goals) %>%
  head(1)
